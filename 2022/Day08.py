...
def get_outside_visible_count(forest: np.ndarray) -> int:
    ...
    for tree_line_id, tree_line in enumerate(forest[1:-1], start=1):
        ...
        visible_indices.update(
            set((tree_line_id, tree_id) for tree_id in visible_tree_indices)
        )
        # r to l
        visible_tree_indices = get_visible_indices(tree_line[::-1])
        visible_indices.update(
            set((tree_line_id, width - 1 - tree_id) for tree_id in visible_tree_indices)
        )
    ...
    for tree_line_id in range(1, height - 1):
        ...
        visible_indices.update(
            set((tree_id, tree_line_id) for tree_id in visible_tree_indices)
        )
        # bottom to top
        visible_tree_indices = get_visible_indices(forest[::-1, tree_line_id])
        visible_indices.update(
            set(
                (height - 1 - tree_id, tree_line_id) for tree_id in visible_tree_indices
            )
        )
...
def get_position_scenic_score(forest: np.ndarray, position: Tuple[int, int]) -> int:
    ...
    down = get_lower_than(
        forest[position[0], position[1] + 1 : forest_height], height=self_height
    )
    left = get_lower_than(
        forest[: position[0] :, position[1]][::-1], height=self_height
    )
    right = get_lower_than(
        forest[position[0] + 1 : forest_width, position[1]], height=self_height
    )
