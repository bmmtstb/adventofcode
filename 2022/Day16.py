...
def load_data(
    ...
    non_zero_flows = {
        key: value for key, value in flows.items() if value > 0 or key == "AA"
    }
...
def get_from_to(edges: Edges, tunnel: TunnelSystem) -> TravelGuide:
    ...
    travel_guide: TravelGuide = {
        key: {sub_key: int(1e10) for sub_key in tunnel.keys()} for key in tunnel.keys()
    }
    ...
    for key_k in tunnel.keys():
        for key_i in tunnel.keys():
            for key_j in tunnel.keys():
                if (
                    travel_guide[key_i][key_j]
                    > travel_guide[key_i][key_k] + travel_guide[key_k][key_j]
                ):
                    travel_guide[key_i][key_j] = (
                        travel_guide[key_i][key_k] + travel_guide[key_k][key_j]
                    )
    assert all(
        all(sub_key_value < int(1e10) for sub_key_value in travel_guide[key].values())
        for key in travel_guide.keys()
    )
    return travel_guide
...
def get_max_released_pressure(
    ...
    for next_node in flows.keys():
        ...
        possible_flow_recursive = get_max_released_pressure(
            ...
            + [next_node],  # this one is opened too recursively
        )
...
def get_max_released_pressure_two_person(
    travel_guide: TravelGuide,
    valve_flows: FlowRates,
    me_starting_node: str = "AA",
    elephant_starting_node: str = "AA",
    me_minutes_left: int = 26,
    elephant_minutes_left: int = 26,
    opened_valves: List[str] = None,
) -> int:
    """
    ...
    if me_minutes_left >= elephant_minutes_left:
        ...
        for next_node in valve_flows.keys():
            ...
            if time_to_open_valve > me_minutes_left:
                continue
            # flow times the total amount valve can be open for
            possible_flow_released = valve_flows[next_node] * (
                me_minutes_left - time_to_open_valve
            )

            possible_flow_recursive = get_max_released_pressure_two_person(
                travel_guide=travel_guide,
                valve_flows=valve_flows,
                me_starting_node=next_node,
                elephant_starting_node=elephant_starting_node,
                me_minutes_left=me_minutes_left - time_to_open_valve,
                elephant_minutes_left=elephant_minutes_left,
                opened_valves=opened_valves
                + [next_node],  # this one is opened too recursively
            )
        ...
        for next_node in valve_flows.keys():
            ...
            time_to_open_valve: int = (
                travel_guide[elephant_starting_node][next_node] + 1
            )
            # break if there is no more time
            ...
            # flow times the total amount valve can be open for
            possible_flow_released = valve_flows[next_node] * (
                elephant_minutes_left - time_to_open_valve
            )

            possible_flow_recursive = get_max_released_pressure_two_person(
                travel_guide=travel_guide,
                valve_flows=valve_flows,
                me_starting_node=me_starting_node,
                elephant_starting_node=next_node,
                me_minutes_left=me_minutes_left,
                elephant_minutes_left=elephant_minutes_left - time_to_open_valve,
                opened_valves=opened_valves
                + [next_node],  # this one is opened too recursively
            )
...
class Test2022Day16(unittest.TestCase):
    test_tunnel, test_flow, test_non_zero_flow, test_edges, test_trave_guide = (
        load_data("data/16-test.txt")
    )

    def test_get_max_released_pressure(self):
        self.assertEqual(
            get_max_released_pressure(self.test_trave_guide, self.test_non_zero_flow),
            1651,
        )

    def test_get_max_released_pressure_with_two_people(self):
        self.assertEqual(
            get_max_released_pressure_two_person(
                self.test_trave_guide, self.test_non_zero_flow
            ),
            1707,
        )
...
if __name__ == "__main__":
    ...
    (
        puzzle_tunnel,
        puzzle_flow,
        puzzle_non_zero_flow,
        puzzle_edges,
        puzzle_trave_guide,
    ) = load_data("data/16.txt")
    print(
        "Part 1): ", get_max_released_pressure(puzzle_trave_guide, puzzle_non_zero_flow)
    )
    print(
        "Part 2): ",
        get_max_released_pressure_two_person(puzzle_trave_guide, puzzle_non_zero_flow),
    )
    print("End Main 16<<<")
