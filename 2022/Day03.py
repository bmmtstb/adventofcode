...
def get_group_priority_scores(rucksacks: List[str]) -> int:
    ...
    return sum(
        get_group_item_priority(rucksacks[start_idx : start_idx + 3])
        for start_idx in range(0, len(rucksacks), 3)
    )
...
class Test2022Day03(unittest.TestCase):
    ...
    def test_split_rucksack(self):
        self.assertEqual(
            split_rucksack("vJrwpWtwJgWrhcsFMMfFFhFp"), ["vJrwpWtwJgWr", "hcsFMMfFFhFp"]
        )
    ...
    def test_get_sharing_priority(self):
        for idx, priority in [
            ...
            with self.subTest(msg=f"Rucksack: {self.test_rucksacks[idx]}"):
                self.assertEqual(
                    get_sharing_priority(split_rucksack(self.test_rucksacks[idx])),
                    priority,
                )
