...
class Elves:
    ...
    def has_any_neighbors_in_direction(self, pos: ElfPosition, direction: int) -> bool:
        ...
        if direction == 0:
            return any(
                other_elf[1] == pos[1] - 1 and pos[0] - 1 <= other_elf[0] <= pos[0] + 1
                for other_elf in self.elves
                if pos is not other_elf
            )
        elif direction == 1:
            return any(
                other_elf[1] == pos[1] + 1 and pos[0] - 1 <= other_elf[0] <= pos[0] + 1
            ...
            return any(
                other_elf[0] == pos[0] - 1 and pos[1] - 1 <= other_elf[1] <= pos[1] + 1
                for other_elf in self.elves
                if pos is not other_elf
            )
        elif direction == 3:
    ...
    def has_at_least_one_neighbor(self, pos: ElfPosition) -> bool:
        ...
        return any(
            ...
            and pos[1] - 1 <= other_elf[1] <= pos[1] + 1
            for other_elf in self.elves
            if other_elf != pos
        )
    ...
    def has_neighbors_in_dir(self, pos: ElfPosition) -> Tuple[bool, bool, bool, bool]:
        ...
        for other_elf in self.elves:
            if (
                not N
                and other_elf[1] == pos[1] - 1
                and pos[0] - 1 <= other_elf[0] <= pos[0] + 1
            ):
                N = True
            if (
                not S
    ...
    def draw_board(self) -> str:
        ...
        for row_id in range(min_[1], max_[1] + 1):
            img += (
                "".join(
                    "#" if (x, row_id) in self.elves else "."
    ...
    def run_simulation(self, n: int = -1) -> int:
        ...
        while iteration != n:
                ...
                for dir_i in range(0, 4):
                    ...
                    if not neighbors[(offset + dir_i) % 4]:
                        new_positions.append(
                            tuple_add_tuple(elf, MOVE_DIRECTION[(offset + dir_i) % 4])
                        )
                        elves_moved = True
                        break
            ...
            for testing_index, new_pos in enumerate(new_positions):
                ...
                for other_idx, other_pos in enumerate(
                    new_positions[testing_index + 1 :], start=testing_index + 1
                ):
                    # skip already duplicates
...
class Test2022Day23(unittest.TestCase):
    ...
    def test_setup(self):
        self.assertEqual(
            deepcopy(self.test_elves_small.elves),
            [(2, 1), (3, 1), (2, 2), (2, 4), (3, 4)],
        )

    def test_get_free_spaces_in_rectangle(self):
        self.assertEqual(
            deepcopy(self.test_elves_small).get_free_spaces_in_rectangle(), 3
        )
    ...
    def test_has_any_neighbors_in_direction(self):
        for elves, elf_pos, testing_direction, has_neighbors in [
            ...
            with self.subTest(msg=f"pos: {elf_pos}"):
                self.assertEqual(
                    elves.has_any_neighbors_in_direction(elf_pos, testing_direction),
                    has_neighbors,
                )
    ...
    def test_has_at_least_one_neighbor(self):
        for elves, elf_pos, has_neighbors in [
            ...
            with self.subTest(msg=f"pos: {elf_pos}"):
                self.assertEqual(
                    elves.has_at_least_one_neighbor(elf_pos), has_neighbors
                )
