...
def solve_graph(dep_graph, goal="FUEL", start="ORE"):
    ...
    while any(
        start != key and current_goal_node[0][key] > 0
        for key in current_goal_node[0].keys()
    ):
        ...
        for key in list(current_goal_node[0].keys()):
            if key != start:
                ...
                if len(possiple_solutions) < 1:
                    raise Exception(
                        "The key {} was not found in the dependency tree".format(key)
                    )
                elif len(possiple_solutions) > 1:
                    raise Exception(
                        "The key {} appears in more than one dependency path".format(
                            key
                        )
                    )
                else:
                    ...
                    reaction_multiplicator = math.ceil(
                        necessary_educt / possible_product
                    )
                    overproduction = necessary_educt - (
                        reaction_multiplicator * possible_product
                    )
...
class Test2019Day14(unittest.TestCase):
    def test_parse_dep_graph(self):
        for dep, result in [
            [["9 ORE => 2 A"], [({"ORE": 9}, {"A": 2})]],
            [["9 ORE, 100 B => 2 A"], [({"ORE": 9, "B": 100}, {"A": 2})]],
            [
                ["9 ORE => 2 A, 10 B, 100 C"],
                [({"ORE": 9}, {"A": 2, "B": 10, "C": 100})],
            ],
            [
                ["9 ORE => 2 A", "9 ORE => 2 A"],
                [({"ORE": 9}, {"A": 2}), ({"ORE": 9}, {"A": 2})],
    ...
    def test_ore_to_fuel_ratio(self):
        for fuel, dependencies in [
    ...
    def test_trillion_to_fuel_ratio(self):
        for fuel, dependencies in [
