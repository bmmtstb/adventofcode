...
def code_to_coordinates(wire):
    """
    Transform wire language to list of coordinates, where a cable lays
    :param wire: list of actions / directions the wire performs
    :return: coords, list of tuples of wire coordinates
    """
    coords = [(0, 0)]
    for action in wire:
        last = coords[-1]
        if action.startswith("U"):
            [coords.append((last[0], last[1] + x)) for x in range(1, int(action[1:]) + 1)]
        elif action.startswith("D"):
            [coords.append((last[0], last[1] - x)) for x in range(1, int(action[1:]) + 1)]
        elif action.startswith("R"):
            [coords.append((last[0] + x, last[1])) for x in range(1, int(action[1:]) + 1)]
