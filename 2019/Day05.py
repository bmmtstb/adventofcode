...
def set_value_indirect(code, pointer, mode, offset, base, value, op):
    ...
    # can't be direct mode
        ...
        raise Exception(
            "In op mode {} the parameter mode {} is not supported".format(op, mode)
        )
...
def run_intcode_program(
    ...
    while intcode[instruction_pointer] != 99 and instruction_pointer < len(intcode):
        ...
        if op == 1:  # add
            first = get_value_indirect(
                intcode, instruction_pointer, c, 1, relative_base
            )
            second = get_value_indirect(
                intcode, instruction_pointer, b, 2, relative_base
            )
            # intcode[intcode[instruction_pointer + 3]] = first + second
            value = first + second
            set_value_indirect(
                intcode, instruction_pointer, a, 3, relative_base, value, op
            )
            ...
            set_value_indirect(
                intcode, instruction_pointer, c, 1, relative_base, value, op
            )
            ...
            raise ValueError(
                "Something went wrong, opcode {} is not expected.".format(op)
            )
    ...
    def test_final_results(self):
        for start_value, result in [
            ...
            with self.subTest():
                self.assertEqual(
                    run_intcode_program(
                        puzzle_input.copy(), start_value, show_output=False
                    )[0][-1],
                    result,
                )
