    ...
    with open(filepath) as file:
        for line in file.readlines():
            ...
            if line.startswith("mem"):
                ...
                raise Exception(
                    "Bad parameter in line: {} while reading file {}".format(
                        line, filepath
                    )
                )
    return data
...
def run_init_program(code: List[tuple]) -> Dict[int, str]:
    ...
    for cmd in code:
        if cmd[0] == "mask":
            ...
            masked = "0b" + "".join(
                map(lambda m, b: b if m == "X" else m, curr_mask, byte)
            )
            mem[cmd[1]] = masked
        else:
...
def run_variant2(code: List[tuple]) -> Dict[int, int]:
    ...
    for cmd in code:
        if cmd[0] == "mask":
            ...
            masked = "".join(
                map(
                    lambda mask, b: "X" if mask == "X" else b if mask == "0" else "1",
                    curr_mask,
                    byte,
                )
            )
...
class Test2020Day14(unittest.TestCase):
    def test_v1(self):
        self.assertEqual(
            run_init_program(load_data("data/14-test.txt")),
            {
                7: "0b000000000000000000000000000001100101",
                8: "0b000000000000000000000000000001000000",
            },
        )
