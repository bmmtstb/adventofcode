...
def get_possible_strings(messages: List[str], rule: int, rules: Dict[int, Union[str, List[Tuple[int, ...]]]], max_recursion_depth: int = 0,
                         recursion=None) -> Set[str]:
    ...
    if type(curr_rules) == str:
        ...
        for subrules in curr_rules:
            ...
            for i, subrule in enumerate(subrules):
                possible_substrings.append(get_possible_strings(messages, subrule, rules, max_recursion_depth=max_recursion_depth, recursion=deepcopy(recursion)))
            ...
            possible_rules.update(set("".join(c for c in l) for l in itertools.product(*possible_substrings)))
        # remove items that are no longer possible (to decrease size of tree)
        possible_rules = set(pr for pr in possible_rules if any(pr in msg for msg in messages))
        return possible_rules


def check_rule_on_messages(messages: List[str], rules: Dict[int, Union[str, List[Tuple[int, ...]]]], rule: int = 0, max_rec: int = 6) \
        -> List[bool]:
...
def check_rules_on_single_messages(messages: List[str], rules: Dict[int, Union[str, List[Tuple[int, ...]]]], rule: int = 0) \
        -> List[bool]:
    ...
    for message in messages:
        if message not in solutions:
            ...
            solutions.update(get_possible_strings([message], rule, rules, max_recursion_depth=10))
