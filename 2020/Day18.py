...
def split_formulars(str_equations: List[str]) -> List[List[Union[str, int]]]:
    ...
    for line in str_equations:
        results.append([c if c in ["+", "*", "(", ")"] else int(c) for c in line if c != " "])
    return results
...
def solve_l_to_r(equation: List[Union[str, int]], l_to_r: bool = True):
        ...
        for i in range(start, len(equation)):
            if equation[i] == "(":
                ...
                if len(opening_indices) == 0:
                    raise Exception("No opening bracket for closing bracket in {}".format(equation))
                else:
                    open_idx = opening_indices.pop(-1)
                    new_val = solve_l_to_r_w_o_brackets(equation[open_idx + 1:i]) if l_to_r else solve_sum_before_prod_w_o_brackets(equation[open_idx + 1:i])
    ...
    # solve w.o. brackets
    value = solve_l_to_r_w_o_brackets(equation) if l_to_r else solve_sum_before_prod_w_o_brackets(equation)
    return value


def solve_formulars(equations: List[List[Union[str, int]]], l_to_r: bool = True) -> List[int]:
