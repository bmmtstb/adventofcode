...
def generate_possible_paths(
    ...
    init_path: List[str] = None,
    single_multiple_times: bool = False,
) -> List[List[str]]:
    ...
    for next_node in cave_system[start]:
        ...
        if (
            next_node.islower()
            and next_node in curr_path
            and (
                (not single_multiple_times)
                or (
                    # case part 2 -> cont if char is in path iff no char appears twice already
                    single_multiple_times
                    and any(
                        curr_path.count(pos) > 1 for pos in curr_path if pos.islower()
                    )
                )
            )
        ):
        ...
        sub_possibilities = generate_possible_paths(
            cave_system,
            start=next_node,
            init_path=deepcopy(curr_path),
            end=end,
            single_multiple_times=single_multiple_times,
        )
    ...
    def test_distinct_length(self):
        for fp, nof, smt in [
            ...
            with self.subTest():
                ...
                self.assertEqual(
                    len(generate_possible_paths(poss_path, single_multiple_times=smt)),
                    nof,
                )
...
if __name__ == "__main__":
    ...
    print(
        "Part 2): ",
        len(
            generate_possible_paths(deepcopy(puzzle_input), single_multiple_times=True)
        ),
    )
    print("End Main 12<<<")
