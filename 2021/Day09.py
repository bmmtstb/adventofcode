...
def get_lowest_adjacent(
    height_map: Map, x: int, y: int, exclude: List[Tuple[int, int]] = None
) -> int:
...
def find_low_points(height_map: Map) -> List[Tuple[int, int, int]]:
    ...
    for curr_y in range(len(height_map)):
        for curr_x in range(len(height_map[0])):
            if height_map[curr_y][curr_x] < get_lowest_adjacent(
                height_map, curr_x, curr_y
            ):
...
def get_basin_size(height_map: Map, x: int, y: int) -> int:
    ...
    while new_added:
        ...
        for point in basin_points:
            ...
            if (
                get_lowest_adjacent(
                    height_map, point[0], point[1], exclude=basin_points
                )
                < 9
            ):
                ...
                for neighbor in neighbors:
                    ...
                    if (
                        neighbor_coord not in basin_points
                        and 0 <= neighbor_coord[0] < len(height_map[0])
                        and 0 <= neighbor_coord[1] < len(height_map)
                        and height_map[neighbor_coord[1]][neighbor_coord[0]] < 9
                    ):
...
if __name__ == "__main__":
    ...
    basin_sizes = [
        get_basin_size(puzzle_input, line[1], line[2]) for line in low_points
    ]
