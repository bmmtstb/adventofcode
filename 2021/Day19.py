...
def match_all_scanners(scanners: Scanners, limit: int = 12) -> (Coords, Coords):
    ...
    while len(remaining_scanners) > 0:
        ...
        try:
            all_beacons, new_scanner = match_two_scanners(
                coords1=all_beacons, coords2=remaining_scanners[i], limit=limit
            )
...
def match_two_scanners(
    coords1: Coords, coords2: Coords, limit: int = 12
) -> (Coords, Coord):
    """
