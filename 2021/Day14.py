...
def perform_pair_insertion_faster(
    start: str, rules: Dict[str, str], nof_steps: int
) -> int:
...
class Test2021Day14(unittest.TestCase):
    ...
    def test_after_n_steps(self):
        for n, res_str in [
            ...
            with self.subTest():
                self.assertEqual(
                    perform_pair_insertion(
                        self.polymer_start, self.pair_insertion_rule.copy(), n
                    ),
                    res_str,
                )
        ...
        for steps, length in [
            ...
            with self.subTest():
                self.assertEqual(
                    len(
                        perform_pair_insertion(
                            self.polymer_start, self.pair_insertion_rule.copy(), steps
                        )
                    ),
                    length,
                )
...
if __name__ == "__main__":
    ...
    puzzle_polymerase_2 = perform_pair_insertion_faster(
        puzzle_template, puzzle_rules.copy(), 40
    )
