...
def repeat_risk_map(rm: Map) -> Map:
    ...
    # repeat
    rm = [
        deepcopy(row) + deepcopy(row) + deepcopy(row) + deepcopy(row) + deepcopy(row)
        for row in rm
    ]
    ...
    for x in range(0, 5 * size_x):
        ...
        for y in range(0, 5 * size_y):
            ...
            rm[y][x] = (
                (rm[y][x] + factor) % 9 if rm[y][x] + factor > 9 else rm[y][x] + factor
            )
    return rm
...
def a_star(risk_map: Map, start: Node, goal: Node) -> Path:
    ...
    def get_neighbors(curr: Node) -> Path:
        ...
        if curr[0] - 1 >= 0:
            neighbors.append(
                (curr[0] - 1, curr[1], int(risk_map[curr[1]][curr[0] - 1]))
            )
        if curr[1] - 1 >= 0:
            neighbors.append(
                (curr[0], curr[1] - 1, int(risk_map[curr[1] - 1][curr[0]]))
            )
        if curr[0] + 1 < size_x:
            neighbors.append(
                (curr[0] + 1, curr[1], int(risk_map[curr[1]][curr[0] + 1]))
    ...
    while len(open_list) > 0:
        ...
        for neighbor_node in get_neighbors(current):
            ...
            cost_through_curr = (
                node_lowest_score[current[1]][current[0]] + neighbor_node[2]
            )
            if (
                cost_through_curr
                < node_lowest_score[neighbor_node[1]][neighbor_node[0]]
            ):
                # This path to neighbor_node is better than any previous one. Record it!
                came_from[neighbor_node] = current
                node_lowest_score[neighbor_node[1]][
                    neighbor_node[0]
                ] = cost_through_curr
                expected_score[neighbor_node[1]][neighbor_node[0]] = (
                    cost_through_curr + heuristic(neighbor_node)
                )
...
class Test2021Day15(unittest.TestCase):
    def test_astar_example(self):
        test_field: Map = read_lines_as_list(
            "data/15-test.txt", instance_type=int, split="every"
        )
    ...
    def test_repeat(self):
        ...
        repeated: Map = read_lines_as_list(
            "data/15-test-repeated.txt", instance_type=int, split="every"
        )
...
if __name__ == "__main__":
    print(">>> Start Main 15:")
    puzzle_input: Map = read_lines_as_list(
        "data/15.txt", instance_type=int, split="every"
    )
    start: Node = (0, 0, int(puzzle_input[0][0]))
    goal: Node = (
        len(puzzle_input[0]) - 1,
        len(puzzle_input) - 1,
        int(puzzle_input[-1][-1]),
    )
