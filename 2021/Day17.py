...
def probe_hits_target(
    init_vel: Velocity, target_area: TargetAreaBorders, init_pos: Position = (0, 0)
) -> (bool, int):
    """Check if a probe hits the given target area"""
    target_x_range = range(
        target_area[0][0],
        target_area[0][1] + 1 if target_area[0][1] > target_area[0][0] else -1,
        1 if target_area[0][1] > target_area[0][0] else -1,
    )
    target_y_range = range(
        target_area[1][0],
        target_area[1][1] + 1 if target_area[1][1] > target_area[1][0] else -1,
        1 if target_area[1][1] > target_area[1][0] else -1,
    )
...
def find_max_height(
    target_area: TargetAreaBorders, init_pos: Position = (0, 0)
) -> (Velocity, int):
    ...
    velocities = [
        (vx, vy)
        for vx in range(1, int(0.5 * target_area[0][1] + 1))
        for vy in range(-abs(target_area[1][0]), target_area[0][1] + 1)
    ]
...
def find_every_possible_velocity(
    target_area: TargetAreaBorders, init_pos: Position = (0, 0)
) -> List[Velocity]:
    """st"""
    ...
    velocities = [
        (vx, vy)
        for vx in range(1, target_area[0][1] + 1)
        for vy in range(-abs(target_area[1][0]), target_area[0][1] + 1)
    ]
    for vel in velocities:
